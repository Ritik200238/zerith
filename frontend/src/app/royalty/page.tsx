            ...
            <article key={r.id} style={{ background: "var(--bg-card)", border: "1px dashed var(--border-dash)", borderRadius: 4 }} className="p-4">
              ...
              <div className="mt-3 grid md:grid-cols-2 gap-2 text-[11px]">
                {r.recipients.map((addr, i) => (
                  <div key={i} className="rounded bg-[var(--bg-alt)]/40 p-2 flex items-center justify-between">
                ...
                <>
                  ...
                  <div className="space-y-2">
                    {recipients.map((r, i) => (
                      <div key={i} className="flex items-center gap-2">
                        ...
                        <input value={r.bps} onChange={(e) => updateRecipientRow(i, "bps", e.target.value)} placeholder="bps"
                          ...
                          className="w-20 bg-[var(--bg-alt)] rounded px-2.5 py-1.5 text-[11px] text-[var(--text)] outline-none focus:ring-1 ring-[var(--text)]" />
                        {recipients.length > 1 && (
                    ...
                    <button onClick={addRecipientRow} disabled={recipients.length >= 10}
                      className="w-full py-1.5 rounded text-[11px] text-[var(--text)] hover:bg-[var(--text)]/10 transition-colors disabled:opacity-40">
                      + Add recipient ({recipients.length}/10)
                    </button>
                  </div>
                  <div className="rounded bg-[var(--bg-alt)]/40 p-2.5 flex items-center justify-between text-xs">
