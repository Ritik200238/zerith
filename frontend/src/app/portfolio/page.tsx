  ...
  if (!deployed) {
    return (
      <main className="mx-auto max-w-[1180px] px-5 md:px-10 py-12 md:py-16 font-body" style={{ background: "var(--bg)", color: "var(--text)" }}>
        <ComingSoonBanner feature="Portfolio Tracker" shipDate="Wave 4 deploy" />
      </main>
    );
  }
              ...
              {computeOpen && (
                <>
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => setComputeOpen(false)} aria-label="Close modal" className="text-[var(--text-muted)] hover:text-[var(--text-secondary)] p-1 rounded-lg hover:bg-white/5">
                      <X size={18} />
                    </button>
                  </div>
